...
DEFAULT_PROJECT_NAME="git-mcp-local-docker"
...
prepare_local_mount_dirs() {
  local root="${SCRIPT_DIR}/working/local-docker"
  mkdir -p \
    "${root}/logs" \
    "${root}/data" \
    "${root}/data/audit" \
  ...
  chmod 0777 \
    ...
    "${root}/data/workspaces" \
    "${root}/certs" || true
  for log_name in api.log web.log mcp.log a2a.log audit.log.jsonl audit-integrity.log; do
    : > "${root}/logs/${log_name}"
    chmod 0666 "${root}/logs/${log_name}" || true
  done
}
...
resolve_compose_env_file() {
  local source_env="$1"
  local resolved_env="${STATE_DIR}/$(basename "${source_env}").compose-resolved"

  python3 - "$source_env" "$resolved_env" <<'PY'
import os
import re
import sys
...
def build_client() -> VaultClient | None:
...
def resolve_value(raw_value: str, client: VaultClient | None) -> str:
    stripped = raw_value.strip()
    match = VAULT_PATTERN.match(stripped)
    if match is None:
        return raw_value
    if client is None:
        raise SystemExit(
            "Vault bootstrap variables are required to resolve Docker env placeholders. "
    ...
    if isinstance(resolved, (str, int, float, bool)):
        ...
        if text:
            return text
    raise SystemExit(f"Unable to resolve Vault placeholder: {match.group(1)}")
...
for raw_line in source_path.read_text(encoding="utf-8").splitlines():
    ...
    lines.append(f"{key}={resolve_value(value, client)}")
...
compose_cmd() {
  ...
  shift 3
  local compose_env
  compose_env="$(resolve_compose_env_file "$runtime_env")"
  ENV_FILE="$compose_env" docker compose -f "$compose_file" --project-name "$project_name" "${COMPOSE_PROFILE_ARGS[@]}" --env-file "$compose_env" "$@"
}
