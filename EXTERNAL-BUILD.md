...
|----------|--------------|
| Linux    | Docker 24+ with BuildKit; or Python 3.13 for the pure-source path |
| macOS    | Docker Desktop 4.x (BuildKit on by default); or Python 3.13 (`brew install python@3.13`) |
...
`pyproject.toml` (py3.13) with `--index-url` pointed at that single index.
