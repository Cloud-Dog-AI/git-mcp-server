...
def _index_response(ui_root: Path) -> Response:
    """Return index.html or a deterministic readiness page when UI assets are absent."""
    index_path = ui_root / "index.html"
    if not index_path.exists():
        return HTMLResponse(
            "<!doctype html><title>git-mcp-server</title><h1>git-mcp-server ready</h1>",
            status_code=200,
        )
...
def register_web_ui(app: FastAPI, config: GlobalConfigModel) -> None:
    ...
    async def runtime_config(request: Request) -> Response:
        ...
        # Browser MCP path: Traefik routes /mcp/* to the MCP server directly.
        # Do NOT use config.mcp_server.base_path (internal process path prefix).
        mcp_path = ""
        env = cfg.get("ENV", "dev")
