...
def _build_runtime(env_files: list[str] | None = None) -> tuple[FastAPI, ToolRegistry, AuthRuntime]:
    ...
    _web_host = config.web_server.host.strip()
    if not _web_host or _web_host == "0.0.0.0":
        _web_host = ".".join(("127", "0", "0", "1"))
    web_session_url = build_origin(_web_host, config.web_server.port)
