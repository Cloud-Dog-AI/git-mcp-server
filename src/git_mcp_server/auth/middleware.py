...
from git_mcp_server.http_client import request_json
...
def install_auth_middleware(
    ...
    *,
    ...
    _web_session_url = web_session_url.rstrip("/") if web_session_url else ""
    ...
    class CompatAuthMiddleware(BaseHTTPMiddleware):
        ...
        async def dispatch(self, request: Request, call_next) -> Response:  # type: ignore[override]
            ...
            # Cookie-based web session fallback: validate against the web server.
            session_token = request.cookies.get(_cookie_name, "").strip()
            if session_token and _web_session_url:
                try:
                    resp = await request_json(
                        "GET",
                        f"{_web_session_url}/auth/me",
                        cookies={_cookie_name: session_token},
                        timeout=5.0,
                    )
                    if resp.status_code == 200:
                        ...
                        user = data.get("user")
                        if user is not None:
                            request.state.web_session_user = user
                            return await call_next(request)
                except Exception:  # noqa: BLE001
                    logger.debug("Cookie session validation failed", exc_info=True)
