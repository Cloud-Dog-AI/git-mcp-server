...
BOOTSTRAP_ENV_FILE="${GIT_MCP_BOOTSTRAP_ENV_FILE:-}"
if [[ -n "${BOOTSTRAP_ENV_FILE}" ]]; then
  if [[ ! -f "${BOOTSTRAP_ENV_FILE}" ]]; then
    echo "GIT_MCP_BOOTSTRAP_ENV_FILE does not point to a readable file" >&2
    exit 1
  fi
  set -a
  # Optional operator-managed bootstrap for local native service control.
