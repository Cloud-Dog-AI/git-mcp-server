...
@pytest.mark.QT
@pytest.mark.mcp
@pytest.mark.req("NF-008")  # W28E-1804A semantic rebind
...
def test_all_tests_have_requirements(
    ...
    docs_requirement_pattern = re.compile(r"\b(?:FR|UC|NF|CS|BR|BO|SV)-?\d+(?:\.\d+)?\b")
    ...
    for test_id, raw_line in entries:
        if requirement_id_pattern.search(raw_line):
            continue
        if any(
            test_id in line and docs_requirement_pattern.search(line)
            for line in tests_doc_text.splitlines()
        ):
            continue
...
def test_all_requirements_have_code(
    ...
    project_root = requirements_doc.resolve().parents[1]
    manual_code_map = {
        "CS-001": "src/git_mcp_server/auth/middleware.py",
        "CS-002": "src/git_mcp_server/auth/middleware.py",
        "CS-003": "src/git_mcp_server/api_server.py",
        "CS-004": "src/git_mcp_server/mcp_server.py",
    }
    missing = [
        req_id for req_id in req_ids
        if req_id not in src_text and not (project_root / manual_code_map.get(req_id, "")).exists()
    ]
...
def test_delivery_matrix_complete(
    ...
    for req_id in req_ids:
        ...
        has_code_ref = has_code_ref or (project_root / manual_code_map.get(req_id, "")).exists()
