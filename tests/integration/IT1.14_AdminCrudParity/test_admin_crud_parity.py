...
def test_admin_mcp_tool_parity(integration_server: dict[str, str | bool], api_key: str) -> None:
    """Requirements: FR-15, FR-17, CFG-08, CFG-09, CFG-10, CFG-11."""
    mcp_base = str(integration_server["mcp_url"]).rstrip("/")
    # /mcp is default-deny since W28C-1705 GM1 (anon -> 401, locked by UT1.63). Authenticate
    # every admin MCP tool call with the seed api-key (admin); this test predated the gate.
    h = {"x-api-key": api_key}
    ...
    listed_users = httpx.post(f"{mcp_base}/mcp/tools/admin_user_list", json={}, headers=h, timeout=10.0)
    assert listed_users.status_code == 200
    ...
    listed_groups = httpx.post(f"{mcp_base}/mcp/tools/admin_group_list", json={}, headers=h, timeout=10.0)
    ...
    listed_keys = httpx.post(f"{mcp_base}/mcp/tools/admin_api_key_list", json={}, headers=h, timeout=10.0)
    assert listed_keys.status_code == 200
