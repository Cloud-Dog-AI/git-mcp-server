...
def _jwt_service_from_runtime(default_env_file: str = "tests/env-IT") -> JWTTokenService:
    config = None
    for env_file in (os.environ.get("TEST_ENV_FILE", "").strip(), default_env_file):
        ...
        if config is None:
            config = candidate
    assert config is not None, "JWT integration contract test env file could not be resolved"
    ...
    assert secret, "CLOUD_DOG__AUTH__JWT__SECRET must be configured for JWT integration contract tests"
    return JWTTokenService(secret=secret, issuer=issuer, audience=audience)


def test_it_jwt_auth_accepts_valid_bearer_token(integration_server: dict[str, str]) -> None:
    """Requirements: FR-04. UCs: UC-078."""
    base_url = integration_server["base_url"]
    service = _jwt_service_from_runtime()
    token = service.issue("jwt-integration-user", {"roles": ["writer"]}).access_token
...
def test_it_jwt_auth_rejects_invalid_bearer_token(integration_server: dict[str, str]) -> None:
    """Requirements: FR-04. UCs: UC-078."""
    base_url = integration_server["base_url"]
