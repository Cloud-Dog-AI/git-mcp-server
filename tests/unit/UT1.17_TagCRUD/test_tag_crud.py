...
def test_tag_crud(tmp_path: Path) -> None:
    """Requirements: FR-10, FR-11."""
    repo = create_repo(tmp_path / "repo")
    service = TagService(GitRepository(repo.working_tree_dir))
    service.create_tag("v1.0.1")
    assert "v1.0.1" in service.list_tags()
    service.delete_tag("v1.0.1")
    assert "v1.0.1" not in service.list_tags()
